...
public class LabelActivity extends BaseTemplateActivity {
    ...
    protected void onFunc2Click() {
        try {
            ...
            LinkKit.getInstance().getDeviceLabel().labelDelete(requestModel, mConnectSendListener);
        } catch (Exception e) {
            showToast("数据格式不对");
            e.printStackTrace();
        }
    ...
    protected void onFunc1Click() {
        try {
            ...
            LinkKit.getInstance().getDeviceLabel().labelUpdate(requestModel, mConnectSendListener);
        } catch (Exception e) {
            showToast("数据格式不对");
            e.printStackTrace();
        }
