package com.gaozhao.remote.aliyun.alink.devicesdk.demo;
...
import com.gaozhao.remote.aliyun.alink.devicesdk.app.DeviceInfoData;
import com.gaozhao.remote.aliyun.alink.devicesdk.manager.IDemoCallback;
import com.gaozhao.remote.aliyun.alink.devicesdk.manager.InitManager;
...
public class AliyunIotActivity extends BaseActivity implements View.OnClickListener {
...
//        startActivity(intent);
    ...
    private boolean checkReady() {
        if (RemotePaymentApplication.userDevInfoError) {
            showToast("设备三元组信息res/raw/deviceinfo格式错误");
            return false;
        }
        if (!RemotePaymentApplication.isInitDone) {
...
//                testJniLeakWithCoAP();
    ...
    private static ArrayList<DeviceInfoData> getTestDataList() {
        ...
        test6.productKey = RemotePaymentApplication.productKey;
        test6.deviceName = RemotePaymentApplication.deviceName;
        test6.deviceSecret = RemotePaymentApplication.deviceSecret;
    ...
    private void connect() {
        ...
        RemotePaymentApplication.productKey = deviceInfoData.productKey;
        RemotePaymentApplication.deviceName = deviceInfoData.deviceName;
        RemotePaymentApplication.deviceSecret = deviceInfoData.deviceSecret;
        new Thread(new Runnable() {
            @Override
            public void run() {
                InitManager.init(AliyunIotActivity.this, RemotePaymentApplication.productKey, RemotePaymentApplication.deviceName,
                        RemotePaymentApplication.deviceSecret, RemotePaymentApplication.productSecret, new IDemoCallback() {
                            ...
                            public void onInitDone(Object data) {
                                ...
                                RemotePaymentApplication.isInitDone = true;
                                showToast("初始化成功");
                            }
                        });
            }
    ...
    private void deinit() {
        ALog.d(TAG, "deinit");
        RemotePaymentApplication.isInitDone = false;
        new Thread(new Runnable() {
            @Override
            public void run() {
                // 同步接口
    ...
    private void publishTest() {
        try {
            ...
            request.qos = 1;
            request.isRPC = false;
            request.topic = "/" + RemotePaymentApplication.productKey + "/" + RemotePaymentApplication.deviceName + "/user/update";
